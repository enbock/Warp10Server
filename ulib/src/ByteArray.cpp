...
void ByteArray::size(uint64 value)
{
	...
	newLength = value+(BYTEARRAY_CHUNK-(value%BYTEARRAY_CHUNK));
...
* writeUInt
*/
void ByteArray::writeUInt(const uint64 value, int64 min, bool noHeader, int8 type)
{
	char maxExp, byte;
	int i;
	...
	while((value >> (8*maxExp)) > 0 && maxExp < sizeof(uint64)) maxExp++;
...
int64 ByteArray::writeBytes(ByteArray& value, int64 length)
{
	...
	return writeBytes(value._buffer, (uint64)(length?length:value.available()), offset);
}
...
void ByteArray::compress()
{
	...
	err = compress2(compr+ba->length(), &cl, (const Bytef*)_buffer, _length, Z_BEST_COMPRESSION);
...
void ByteArray::uncompress()
{
	...
	err = ::uncompress((Bytef*)ucompr, (uLongf *)&cl, (const Bytef*)_buffer+off, (uLong) sl);
	if(err != Z_OK)
	{
...
{
	if(value < _length) _position = value;
...
int64 ByteArray::available() const
{
...
bool u::ByteArray::splice(int64 offset, int64 length)
{
	...
	for(i=0; i < oldSize - length - offset; i++)
	{
		if(offset + i + length < oldSize)
			...
			break; // end of data block before offset+length
	}
