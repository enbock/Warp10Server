...
WebService::WebService(Manager* manager, String address, int64 port) : Object()
{
	...
	_manager->addEventListener(
			u::Network::Event::NETWORK_REGISTERED, Callback(
					this
					, cb_cast(&WebService::onNetworkRegistered))
	);
	_manager->addEventListener(
			u::Network::Event::LISTENER_CREATED, Callback(
					this
					, cb_cast(&WebService::onListenerCreated))
	);
}
...
void WebService::onListenerCreated(Object* arg)
{
	...
	if(event->networkType == type)
	{
		...
		listener->addEventListener(
				WebEvent::INCOMMING_CONNECTION, Callback(
						this
						, cb_cast(&WebService::onNewConnection))
		);
		listener->addEventListener(
				WebEvent::WILL_CLOSE, Callback(
						this
						, cb_cast(&WebService::onListenerClose))
		);
		listener->listen();
	}
	event->destroy();
...
void WebService::onNewConnection(Object* arg)
{
	...
	connection->addEventListener(
			WebEvent::REQUEST, Callback(this, cb_cast(&WebService::onRequest))
	);
	connection->read();
	event->destroy();
}
