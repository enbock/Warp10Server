...
u::Warp10Server::Warp10Server(Vector<String> arg) : RoomOwner()
{
	...
	_room.addEventListener(ServerEvent::SHUTDOWN
		, Callback(this,cb_cast(&Warp10Server::onShutdown))
	);

	_room.addEventListener(ServerEvent::REGISTER_NETWORK_PLUGIN
...
{
	_room.removeEventListener(ServerEvent::SHUTDOWN
		, Callback(this,cb_cast(&Warp10Server::onShutdown))
	);

	_room.removeEventListener(ServerEvent::REGISTER_NETWORK_PLUGIN
		, Callback(this, cb_cast(&Warp10Server::onRegisterNetworkPlugin))
	);
...
{
	ServerEvent *signal = (ServerEvent*) arg;
	_network.lock();
	_network.registerNetworkPlugin((NetworkServicePlugin*)signal->data);
	_network.unlock();

	ServerEvent sendEvent(ServerEvent::NETWORK_PLUGIN_REGISTERED);
	sendEvent.data = signal->data;
